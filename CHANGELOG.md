...
## v2.57 (2026-03-01) - 音频修复 + 输入优化

### 🐛 Bug修复

- **修复游戏音效失真/杂音**: 将音频提示系统从Windows旧版API（winmm.dll PlaySound）重构为Unity内置AudioSource系统，彻底消除与游戏音频管线的冲突，在任何电脑配置上都不会产生杂音
- **修复快速操作时按键无反应**: 移除了每帧磁盘I/O（DebugHelper.Flush）和控制台日志输出，大幅降低mod在主线程的开销

### 📈 性能优化

- **屏幕阅读器响应速度提升**: Tolk最小调用间隔从50ms降至30ms，移除每秒调用次数限制
- **日志缓冲优化**: 日志刷新间隔从每5次写入改为每50次写入，减少90%的磁盘I/O

### 🔧 技术细节

- AudioCueManager: 两阶段初始化（后台线程生成波形 → 主线程创建Unity AudioClip/AudioSource）
- AudioSource: bypassEffects/bypassListenerEffects/bypassReverbZones确保不受游戏音效处理影响
- 淡入淡出从2ms增加到5ms，消除爆音
...
| 版本 | 日期 | 类型 | 主要变更 |
|------|------|------|----------|
| 2.57 | 2026-03-01 | 修复 | 音频失真修复 + 输入延迟修复 |
